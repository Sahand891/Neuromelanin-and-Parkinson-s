...
July 31, 2020
  ...
  A volcano plot of age correlations was constructed to better visualize the results. This included the -log(p-value of correlation) on the y-axis and the correlation coefficient on the x-axis. Positively correlated ACGs are blue, negatively correlated ACGs are red, and genes not significantly correlated with age are in .
```{r}
library(ggplot2)
library(dplyr)
...
volcano_plot_df$differentially_expressed[volcano_plot_df$Correlation.Coefficient > horz_max_age & volcano_plot_df$BH.corrected.p.value < 0.05] <- "Upregulated"

# If log2Foldchange < horz_min and pvalue < 0.05, set as "Downregulated"
volcano_plot_df$differentially_expressed[volcano_plot_df$Correlation.Coefficient < horz_min_age & volcano_plot_df$BH.corrected.p.value < 0.05] <- "Downregulated"
...
```
  ...
  Among the 739 genes found to be significantly upregulated or downregulated in the SNpc relative to the VTA were two of the target genes:
...
print(paste(dim(mDBR_expr_genes)[1], "genes were aligned to 20000 probes"))
...
September 28, 2020
  ...
  Genes found to be significantly differentially expressed in the SNpc were separated into those that were upregulated and those that were downregulated. They were written to csv files to make it easier to copy and paste the gene lists into enrichment analysis programs.
```{r}
library(dplyr)
...
# Redefine an "age-correlated gene" as any gene whose BH-corrected p-value of correlation witha age < 0.05
...
all_ACG_mDBR_joined <- ACGs %>% inner_join(all_sig_mDBR, by = c("Gene" = "gene.symbol"))
...
STRING DB:
  ...
  While no statistically significant results were found, there were still a number of genes in these sets known to be associated with the melanosome/pigment granule GO term from enrichment analysis on ACGs. These included OCA2 (r = -0.3195436, FC = 0.3787879), RAB9A (-0.3879255, FC = 0.5938242), and YWHAZ (r = -0.3164595, FC = 1.415). The reason for the observation that no significantly enriched pathways is likely due to the fact that there are a relatively small number of genes in each gene set.
